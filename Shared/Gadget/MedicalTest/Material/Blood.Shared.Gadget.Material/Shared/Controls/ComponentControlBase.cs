...
namespace Shared.Gadget.Material
{
  public abstract class TComponentControlBase : ContentControl
  {
    ...
    public void RefreshDesign ()
    {
      ...
      if (Model.ControlModel.ContainsImage) {
        // image
        var modelImage = Model.ControlModel.Model.GetImage ();

        if (modelImage.NotNull ()) {
          var imageSource = rr.Library.Helper.THelper.ByteArrayToBitmapImage (modelImage);
      ...
      // material
      if (string.IsNullOrEmpty (Model.ControlModel.Model.Material).IsFalse ()) {
        var textBlock = new TextBlock ()
        {
          VerticalAlignment = VerticalAlignment.Center,
          Text = Model.ControlModel.Model.Material,
      ...
      if (string.IsNullOrEmpty (Model.ControlModel.Model.Description).IsFalse ()) {
